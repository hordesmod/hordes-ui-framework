...
const killTracker = {
    ...
    addData(pKilled, pKilledLevel, pKilledClass, pKilledFaction, pDead, pDeadLevel, pDeadClass, pDeadFaction, fame, currencyString) {
        ...
        if (this.state.killsInfo.length > this.state.killsDataThreshold) {
            // Find the index of the record with the minimum time
            const indexOfOldestRecord = this.state.killsInfo.reduce((minIndex, currentRecord, currentIndex, array) => {
                return currentRecord.time < array[minIndex].time ? currentIndex : minIndex;
            }, 0);

            // Remove the record with the minimum time
            this.state.killsInfo.splice(indexOfOldestRecord, 1);
        }
    ...
    updateGrid() {
        ...
        if (selectedTab == "latestKills") {
            dataToShow = this.state.killsInfo
        ...
        filteredKills.forEach(killInfo => {
            for (const [key, value] of Object.entries(killInfo)) {
                ...
                if (key == "fame") {
                    ...
                    if (value < 0) {
                        killItem.style.paddingLeft = "5px"
                    }
                } else if (key == "type") {
                    let displayVal
